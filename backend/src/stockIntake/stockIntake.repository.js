...
async function createIntake(restaurantId, userId, items) {
  ...
  await transaction(async (tx) => {
    ...
    for (const item of items) {
      ...
      await tx(
        `INSERT INTO stock_intake_items (id, intake_id, item_id, quantity_added) VALUES ($1, $2, $3, $4)`,
        [uuidv4(), intakeId, item.itemId, qty]
      );
      // Increase stock
      await tx(
        `UPDATE stocks SET quantity = quantity + $1, updated_at = NOW() WHERE item_id = $2 AND restaurant_id = $3`,
        [qty, item.itemId, restaurantId]
      );
    }
  });
  return intakeId;
...
async function listIntakes(restaurantId, start, end) {
  let sql = `SELECT si.id, si.intake_type, si.created_at, sii.item_id, i.name AS item_name, sii.quantity_added
